...
// GetCourse 获取课程信息
func (j *Jwc) GetCourse(params url.Values) ([]Course, error) {
	uri := DOMAIN + "/courseSearchAction.do"
	paramsStr := getParamsStr(params)
	doc, err := j.post(uri, paramsStr)
	if err != nil {
		return nil, err
	}
	...
	doc.Find("#user > tbody > tr").Each(func(i int, s *goquery.Selection) {
		c := Course{}
		v := reflect.ValueOf(&c)
		elem := v.Elem()
		...
		for k := 0; k < elem.NumField(); k++ {
			...
			case "Teachers":
				c.Teachers = teacherParse(strings.TrimSpace(s.Find("td").Eq(k).Text()))
			case "AllWeek":
				allWeek := weekParse(strings.TrimSpace(s.Find("td").Eq(k).Text()))
				elem.Field(k).SetString(allWeek)
			case "Session":
				session, _ := sessionParse(strings.TrimSpace(s.Find("td").Eq(k).Text()))
